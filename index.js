...
const askName = (convo) => {
  convo.ask(`Heyy, welcome to Island hospito! What's your name?`, (payload, convo, data) => {
...
const askFavoriteFood = (convo) => {
  convo.ask(`What's the marathon that you're interested in?`, (payload, convo, data) => {
    const text = payload.message.text;
    convo.set('race', text);
    convo.say(`Got it, Give me more info so that I enroll you for the race ${text}`).then(() => askGender(convo));
  });
};
